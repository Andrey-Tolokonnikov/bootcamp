  ...
  return (
    <div className="flex flex-col gap-3 w-full  lg:max-w-[630px] max-w-[300px]">
      <div className="flex md:justify-between w-full md:flex-row flex-col items-start">
        <h1 className="text-2xl font-bold block">{props.product?.title ?? <Skeleton className="h-7 w-80 rounded-md"/>}</h1>
        {props.product ? (
          <Popover>
      ...
      <p><a href={props?.product?.link ?? "#"} className="text-primary flex items-center gap-1"><Link/>{props.product?.link ?? <Skeleton className="h-5 w-72 rounded-md"/>}</a></p>
      <p className="text-slate-700 dark:text-slate-200 flex items-center gap-1"><User className="inline"/>{props.product?.owner ?? <Skeleton className="h-5 w-72 rounded-md"/>}</p>
